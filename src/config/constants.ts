export const MAX_INPUT_LENGTH = 10000;
export const PASTED_TEXT_FILE_THRESHOLD = 4000;
export const MAX_TEXTAREA_HEIGHT = 200;
export const DEBOUNCE_MS = 150;
export const TITLE_MAX_LENGTH = 40;
export const DEFAULT_TEMPERATURE = 0.7;
export const MAX_TEMPERATURE = 2.0;
export const MIN_TEMPERATURE = 0.0;
export const TEMPERATURE_STEP = 0.1;
export const COLLAPSED_SIDEBAR_WIDTH = 0;
export const DEFAULT_SIDEBAR_WIDTH = 260;
export const DEFAULT_AUX_PANEL_WIDTH = 400;
export const MIN_AUX_PANEL_WIDTH = 240;
export const MAX_AUX_PANEL_WIDTH = 720;
export const DEFAULT_MAX_TOOL_STEPS = 25;
export const MIN_TOOL_STEPS = 1;
export const MAX_TOOL_STEPS_LIMIT = 200;
export const MAX_SUBAGENTS_PER_CALL = 4;
export const MAX_SUBAGENT_DEPTH = 3;
export const MAX_ACTIVE_SUBAGENTS = 8;
export const MAX_TOOL_RESULT_LENGTH = 200_000;
export const MAX_TOOL_CONTEXT_CHARS = 16_000;
export const UNKNOWN_CONTEXT_ASSEMBLY_TOKENS = 32_768;
export const MAX_TOOL_IMAGES = 4;
export const MAX_TOOL_IMAGE_DATA_LENGTH = 7_000_000;
export const MAX_FILE_SIZE_BYTES = 10 * 1024 * 1024;
export const MAX_ATTACHMENTS = 6;
