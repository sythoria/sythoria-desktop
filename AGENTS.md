...
- Gmail, Drive, and Calendar share one encrypted native Google Desktop OAuth client configuration. The renderer receives only its client ID; token exchange resolves the secret in Rust. Complete, unambiguous legacy client pairs migrate from MCP environment storage. Dynamic loopback callbacks require Desktop app credentials, not Web clients.
- Each authorization attempt owns a cancellable listener and a fresh private credential directory. Never overwrite another grant or conventional credential locations in the user's home folder. MCP servers require plaintext credential files; do not describe those files as encrypted or claim service data never leaves the device.
...
- Gmail uses `GMAIL_OAUTH_PATH` plus Node-format `GMAIL_CREDENTIALS_PATH`. Drive and Calendar use local OAuth keys/token paths without `GOOGLE_APPLICATION_CREDENTIALS`. Keep catalog package versions and tool contracts aligned.
- Setup keeps fields visible on failure, uses one inline error without duplicate store toasts, and labels browser authorization “Continue with Google.” Existing installations need reauthorization to adopt the corrected scope and credential configuration.
