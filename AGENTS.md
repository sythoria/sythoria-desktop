...
- **useSearchStore**: `searchConfigs`, `activeSearchId`, `performSearch()`, `fetchUrlContent()`. Web Search in the composer tool menu is an insert action: it adds an inline chip at the caret, and removing that chip removes web search from the draft. Each composer derives its web-search draft scope from its own chip, so main and side-chat chips cannot affect each other. Sending snapshots the active provider on the user message, preserves a readable `[Web Search]` marker that renders as a chip in chat, and clears only the submitted composer while the captured run continues independently.
