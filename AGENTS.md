...
- **useSearchStore**: `searchConfigs`, `activeSearchId`, `isSearchEnabled`, `performSearch()`, `fetchUrlContent()`. Enabling web search inserts a removable inline chip in the prompt editor. Sending snapshots its provider on the user message, preserves a readable `[Web Search]` marker that renders as a chip in chat, and clears the composer toggle while the captured run continues independently.
